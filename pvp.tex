...
\begin{proof}
    ...
    \begin{equation*}
        \Phi^k \coloneqq \mathop{\E}_{x} \left[\abs{\braket{\tar^{x}}{\lab^{x}}}^2\right].
    \end{equation*}
...
We will analyze $\textsc{SubTest}_{\calT}(\ket{V}: \ket{U})$ for any two $m$ qubit states $\ket{V}$ and $\ket{U}$ and any $m-1$ qubit orthonormal basis $\calL$ in which $\ket{U^0}$ and $\ket{U^1}$ are both phase states. (Here, $\ket{U^b}$ denotes the reduced state if one were to measure the first qubit of $\ket{U}$ and observe $b$.) We denote the basis vectors of $\calL$ by $\ket{\ell}$, where $\ell \in [2^{m-1}]$.
Write
\begin{equation*}
...
\begin{proof}
    ...
    As in \Cref{cor:perpy}, let $\ket{e_t}$ be perpendicular to both $\rho^0$ and $\rho^1$ on the Bloch sphere and measure the the $t$th qubit in the basis $\ket{e_t},\ket{\smash{e_t^\perp}}$. Then set $\Pi_{t+1}=\Pi_t \otimes\ketbra{b}{b}\otimes\Id$, where $\ket{b}\in \{\ket{e_t},\ket{\smash{e^\perp_t}}\}$ was the outcome of this measurement. Then $\ket{\smash{\psi^0_{t+1}}}\propto\Pi_{t+1}\ket{\smash{\psi^0}}$ and $\ket{\smash{\psi^1_{t+1}}}\propto\Pi_{t+1}\ket{\smash{\psi^1}}$ and we increase $t$ by $1$. Repeating for all $t$ gives a measurement outcome in a DT basis satisfying the conclusion of \Cref{cor:perpy}. Computing the $t$th measurement and updating the query take $O(1)$ time, so the overall runtime is $O(n)$.
\end{proof}
