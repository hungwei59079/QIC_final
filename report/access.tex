...
\subsection{Access Model and Computational Efficiency}
...
\end{lemma}
\begin{proof}
  The algorithm will initialize $t=0$, $\ket{\smash{\psi^0_t}}=\ket{\smash{\psi^0}}$, $\ket{\smash{\psi^1_t}}=\ket{\smash{\psi^1}}$, and an initially empty tensor product of projections $\Pi_t=1$ that corresponds to the measurement outcomes observed up to the $t^{th}$ single-qubit measurement. While $t<n$, we compute the reduced states $\rho^0_t = \Tr_{[n]\setminus \{t+1\}}(\ketbra{\smash{\psi^0_t}}{\smash{\psi^0_t}})$ and $\rho^1_t = \Tr_{[n]\setminus \{t+1\}}(\ketbra{\smash{\psi^1_t}}{\smash{\psi^1_t}})$. This can be done with single-qubit state reconstruction as follows:

  For $a\in\{0,+,\mathfrak{i}\}$, build the measurement operator
\[
  \{\Pi = \Pi_t \otimes \ketbra{b}{b} \otimes \Id^{\otimes (n - t - 1)}\}_{b \in \{a, a^{\bot}\}}
\]
  Then measure $\bra{\smash{\psi^0}}\Pi\ket{\smash{\psi^0}}\text{ and }\bra{\smash{\psi^1}}\Pi\ket{\smash{\psi^1}}$. Since we get real values back, we can exactly reconstruct $\rho^0_t$ from these three values. Repeat this $N$ times to estimate $\langle \rho_t^0 \mid a\rangle$ with an error of $\frac 1 {\sqrt N}$. Finally project back onto the Bloch ball.
