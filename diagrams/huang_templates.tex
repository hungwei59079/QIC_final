...
\makeatletter
\@ifundefined{ifHuangShowBoundingBoxes}{%
  \newif\ifHuangShowBoundingBoxes
  % Set this to \HuangShowBoundingBoxesfalse to hide diagram guide boxes.
  \HuangShowBoundingBoxesfalse
}{}
\makeatother
...
\providecommand{\HuangUseBoundingBox}[4]{%
  \ifHuangShowBoundingBoxes
    \begingroup
      \pgfmathsetmacro{\huangbbcx}{(#1+#3)/2}
      \pgfmathsetmacro{\huangbbcy}{(#2+#4)/2}
      \draw[red!75!black, densely dashed, line width=0.6pt] (#1,#2) rectangle (#3,#4);
      \draw[red!75!black, opacity=0.45, line width=0.4pt] (\huangbbcx,#2) -- (\huangbbcx,#4);
      \draw[red!75!black, opacity=0.45, line width=0.4pt] (#1,\huangbbcy) -- (#3,\huangbbcy);
    \endgroup
  \fi
  \pgfresetboundingbox
